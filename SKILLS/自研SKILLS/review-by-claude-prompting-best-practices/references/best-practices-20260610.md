...
status: ARCHIVED — 已于 2026-06-10 被新版 best-practices.md(三页合并版)取代,仅作历史对照,勿用于审查。
last_fetched: 2026-05-29
cache_ttl_days: 14
models_covered: [Claude Opus 4.8, Claude Opus 4.7, Claude Opus 4.6, Claude Sonnet 4.6, Claude Haiku 4.5]
note: 这是 Claude 官方 prompt engineering 最佳实践文章的本地缓存。
      时间新鲜度:使用前运行 scripts/check_cache_age.sh(默认 TTL 14 天)。
      内容新鲜度:若执行 skill 的模型或用户提到的 GA Claude 模型不在 models_covered 内,
      即使时间未过期也应视为内容陈旧,提示联网刷新(见 SKILL.md 步骤 1.3)。
-->
