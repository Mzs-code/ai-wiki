...
本 skill 的方法源头。**这是原版 prompt 的忠实存档,供溯源与对照**;日常使用请直接遵循 `SKILL.md`——它在此原版基础上补充了"为什么有效"、实测沉淀的写作心法,以及若干防套路防线。

**溯源**:方法由 Anthropic 的 Amanda Askell 在 2025 年 4 月一次访谈中提出(她常在无聊时让 Claude 讲寓言,久而久之"脑子里装了好多小故事,每个对应一个学科概念——有时学名忘了,故事还记着")。卡兹克(数字生命卡兹克)在其上做了三点优化:① 增加防套路约束;② 把"指定领域、让 AI 随机挑概念"改为"指定一个具体概念";③ 结尾加两道检验题(理解检验 + 迁移检验)。下面是优化后的完整原版。

**出处链接**:卡兹克(数字生命卡兹克)《分享一个很实用的寓言故事 prompt，5 分钟帮你理解任何新概念。》
